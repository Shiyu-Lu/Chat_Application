...
public class ChatHttpServer {
    ...
    public ChatHttpServer() throws Exception {
        ...
        server.createContext("/6",new sendConversationMessageHandler());// TODO ************************************
        server.createContext("/7",new getConversationListHandler());// TODO ************************************
        ...
        public void handle(HttpExchange t) throws IOException {
            ...
            if ("POST".equalsIgnoreCase(requestMethod)) {
                ...
                for(int i=0;i<r.size();i++){
                    ...
                    ConversationMessage conversationMessage = conversation.getMessageList().last();
                    ConversationLastMessageForJson recentMessage = new ConversationLastMessageForJson();
                    recentMessage.setIndex(i);
                    recentMessage.setLastMsgContent(conversationMessage.getContent());
                    recentMessage.setConvUserName(conversation.getName().replace(userName,""));
